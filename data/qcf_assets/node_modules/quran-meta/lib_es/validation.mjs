...
import { isValidAyahId, isValidJuz, isValidManzil, isValidPage, isValidRuku, isValidSurah } from "./typeGuards.mjs";
...
function checkValidSurah(surah, meta) {
	...
	if (!isValidSurah(surah, meta)) throw new RangeError("Surah must be between 1 and " + meta.numSurahs);
}
/**
...
function checkValidSurahAyahPair(surahAyah, data) {
	...
	if (typeof ayah !== "number" || !Number.isInteger(ayah) || ayah < 1 || ayah > getAyahCountInSurah(surah, data)) throw new RangeError("Ayah must be between 1 and " + getAyahCountInSurah(surah, data));
}
/**
...
function checkValidAyahId(ayahId, meta) {
	...
	if (!isValidAyahId(ayahId, meta)) throw new RangeError("Ayah ID must be between 1 and " + meta.numAyahs);
}
/**
...
function checkValidPage(x, meta) {
	...
	if (!isValidPage(x, meta)) throw new RangeError("Page must be between 1 and " + meta.numPages);
}
/**
...
function checkValidJuz(x, meta) {
	...
	if (!isValidJuz(x, meta)) throw new RangeError("Juz must be between 1 and " + meta.numJuzs);
}
/**
...
function checkValidRuku(x, meta) {
	...
	if (!isValidRuku(x, meta)) throw new RangeError("Ruku must be between 1 and " + meta.numRukus);
}
/**
...
function checkValidManzil(x, meta) {
	...
	if (!isValidManzil(x, meta)) throw new RangeError("Manzil must be between 1 and " + meta.numManzils);
}
