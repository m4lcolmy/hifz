...
* ```
*/
function ayahStringSplitter(str, isStrict = true, data) {
	...
	const { meta } = data;
	if (!isValidSurah(surahX, meta)) throw new Error("Invalid ayah number: " + str);
	const surah = surahX;
	let ayahs;
	if (ayahTo) {
...
function string2NumberSplitter(str) {
	...
	if (sr?.groups && Number(sr.groups.surah) > 0) {
...
function string2NumberSplitterStrict(str) {
	...
	if (isNaN(surahX)) throw new TypeError("Error in surah format " + str);
	...
	if (ayahsStr.includes("-")) ayahs = ayahsStr.split("-").map((a) => {
		...
		if (isNaN(ayahX)) throw new TypeError("Error in surah format " + str);
		return ayahX;
	});
	else {
