...
*/
function checkValidSurah(surah, meta) {
	if (typeof surah !== "number" || !Number.isInteger(surah)) throw new TypeError("Ayah ID must be an integer");
	if (!require_typeGuards.isValidSurah(surah, meta)) throw new RangeError("Surah must be between 1 and " + meta.numSurahs);
}
/**
...
*/
function checkValidSurahAyah(surah, ayah, data) {
...
* ```
*/
function checkValidSurahAyahPair(surahAyah, data) {
	const [surah, ayah] = surahAyah;
	checkValidSurah(surah, data.meta);
	if (typeof ayah !== "number" || !Number.isInteger(ayah) || ayah < 1 || ayah > require_getAyahCountInSurah.getAyahCountInSurah(surah, data)) throw new RangeError("Ayah must be between 1 and " + require_getAyahCountInSurah.getAyahCountInSurah(surah, data));
}
/**
...
*/
function checkValidAyahId(ayahId, meta) {
	if (typeof ayahId !== "number" || !Number.isInteger(ayahId)) throw new TypeError("Ayah ID must be an integer");
	if (!require_typeGuards.isValidAyahId(ayahId, meta)) throw new RangeError("Ayah ID must be between 1 and " + meta.numAyahs);
}
/**
...
*/
function checkValidPage(x, meta) {
	if (typeof x !== "number" || !Number.isInteger(x)) throw new TypeError("Page must be an integer");
	if (!require_typeGuards.isValidPage(x, meta)) throw new RangeError("Page must be between 1 and " + meta.numPages);
}
/**
...
*/
function checkValidJuz(x, meta) {
	if (typeof x !== "number" || !Number.isInteger(x)) throw new TypeError("Juz must be an integer");
	if (!require_typeGuards.isValidJuz(x, meta)) throw new RangeError("Juz must be between 1 and " + meta.numJuzs);
}
/**
...
* ```
*/
function checkValidRuku(x, meta) {
	if (typeof x !== "number" || !Number.isInteger(x)) throw new TypeError("Ruku must be an integer");
	if (!require_typeGuards.isValidRuku(x, meta)) throw new RangeError("Ruku must be between 1 and " + meta.numRukus);
}
/**
...
*/
function checkValidManzil(x, meta) {
	if (typeof x !== "number" || !Number.isInteger(x)) throw new TypeError("Manzil must be an integer");
	if (!require_typeGuards.isValidManzil(x, meta)) throw new RangeError("Manzil must be between 1 and " + meta.numManzils);
}
